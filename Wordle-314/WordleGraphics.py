...
CORRECT_COLOR = "#FFA500"        # Orange for correct letters
PRESENT_COLOR = "#ADD8E6"        # Light blue for misplaced letters
...
class WordleSquare:
    ...
    def __init__(self, canvas, row, col):
        ...
        self._frame = canvas.create_rectangle(x0, y0, x1, y1, fill=self._color)
        self._text = canvas.create_text(x0 + SQUARE_SIZE / 2, y0 + SQUARE_SIZE / 2,
                                        text=self._ch, font=SQUARE_FONT)
...
class WordleKey:
    ...
    def __init__(self, canvas, x, y, width, height, label):
        ...
        self._color = UNKNOWN_COLOR  # Initialize keys with unknown color
        font = KEY_FONT
        if label == "ENTER":
            font = ENTER_FONT
        if label == "DELETE":
        ...
        self._frame = canvas.create_polygon(points,
                                            fill=self._color,  # Initialize key color
                                            outline=KEY_COLOR,
                                            smooth=True)
